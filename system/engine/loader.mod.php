...
final class Loader {
	...
	public function model($route) {
		...
		if (!$this->registry->has('model_' . str_replace('/', '_', $route))) {
			...
			} else throw new \Exception('Error: Could not load model ' . $route . '!');
		}
	}
	...
	public function view($route, $data = array()) {
		...
		if ($result && !($result instanceof Exception)) {
		    ...
		    $class = 'Template\\'.$this->registry->get('config')->get('template_engine');
            $template = new $class();
	...
	public function library($route) {
		...
		} else throw new \Exception('Error: Could not load library ' . $route . '!');
	}
	...
	public function helper($route) {
		...
		if (is_file($file)) {
			include_once(modification($file));
		} else throw new \Exception('Error: Could not load helper ' . $route . '!');
	}
	...
	protected function callback($registry, $route) {
		return function($args) use($registry, $route) {
			...
			if ($result && !$result instanceof Exception) {
				...
				} else throw new \Exception('Error: Could not call model/' . $route . '!');
			}
